...
|---|---|
| **asmDB bearer token** | Stored as a **Container Apps secret**, encrypted at rest by the platform, injected as an env var the process reads at startup. Never logged, never returned to the browser, kept out of deployment history by `@secure()`. Since asmDB sends no CORS headers, the browser *cannot* reach it even if it wanted to. |
| **Why not Key Vault** | It was, until deployment. A **management-group policy** in this tenant (`KeyVault_PublicNetwork_Modify`, assignment `MCAPSGovDeployPolicies`) forces `publicNetworkAccess: Disabled` on every vault — it reverted our Bicep's `Enabled` within ten seconds. Reaching a vault privately needs a private endpoint **plus** a VNet-injected Container Apps environment, and an environment's VNet config is **immutable**, so that means recreating it at roughly 5× the running cost. An APIM in front does not help: it fronts HTTP APIs and does not proxy the Key Vault data plane. The honest trade-off accepted here is no central rotation or audit trail, and Contributor on the app can read the value back. |
